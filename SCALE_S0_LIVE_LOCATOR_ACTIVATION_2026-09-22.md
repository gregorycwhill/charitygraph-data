...
> **Current-control-plane addendum (2026-09-26):** This is subordinate mechanism authority only. Its ABN-valued population text is immutable compatibility evidence, not current live subject identity. Current authority is `CG-S0-PO-ATTEMPT20-2026-09-26`; governed locator subject refs and explicit ABN lookup identifiers are in `policies/scale-s0/locator-subject-bindings-v1.yaml`. Attempt 20 is allocated but A3-pending and non-executable; Attempt 19 is immutable superseded history. `reality_slice1` is deprecated non-canonical development-only code, unreachable from the current live S0 CLI/path.
